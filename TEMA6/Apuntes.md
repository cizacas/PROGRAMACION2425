...
- [UNIDAD 6. ESTRUCTURAS DE ALMACENAMIENTO DINÁMICAS - COLECCIONES Y MAPAS](#unidad-6-estructuras-de-almacenamiento-dinámicas---colecciones-y-mapas)
  - [Índice](#índice)
  - [Introducción a trabajar con genéricos](#introducción-a-trabajar-con-genéricos)
    - [Ventajas de los genéricos](#ventajas-de-los-genéricos)
    - [Tipos de datos parametrizados o genéricos](#tipos-de-datos-parametrizados-o-genéricos)
      - [Ejemplo de genéricos en Arrays](#ejemplo-de-genéricos-en-arrays)
  ...
  - [Introducción a las colecciones](#introducción-a-las-colecciones)
  ...
  - [Introducción a los mapas](#introducción-a-los-mapas)
...
Los genéricos son una característica poderosa de muchos lenguajes de programación modernos, incluyendo Java y C#. Permiten definir clases, interfaces y métodos con un tipo de dato genérico, que se especifica cuando se instancian o se utilizan. Esto proporciona una mayor flexibilidad y seguridad en el manejo de tipos de datos.

### Ventajas de los genéricos

1. **Reutilización de Código**: Los genéricos permiten escribir código que puede trabajar con cualquier tipo de dato, lo que reduce la duplicación de código.
2. **Seguridad de Tipos**: Al especificar el tipo de dato en tiempo de compilación, se evitan errores de tipo en tiempo de ejecución.
3. **Rendimiento**: Los genéricos eliminan la necesidad de conversiones de tipo (casting), lo que puede mejorar el rendimiento del programa.


### Tipos de datos parametrizados o genéricos
__Los arrays__ y __las colecciones__ trabajan con tipos de __datos genéricos__.
...
La definición de estas interfaces de java implementadas con genéricos es:
```java
...
```
Implementamos `Comparable` para cualquier clase de objetos que insertemos en cualquier array o colección que pretendamos ordenar.
...
Ejemplo de implementar una clase comparadora para ordenar los estudiantes por orden alfabético de nombres.
```java
...
```
#### Ejemplo de genéricos en Arrays

En muchos lenguajes, los arrays pueden ser genéricos cuando se implementan las interfaces `Comparable` y `Comparator`. Esto permite ordenar y comparar elementos de un array de manera flexible y segura. Por ejemplo:
...
Los mapas son estructuras de datos que permiten almacenar pares de elementos en forma de clave y valor. A diferencia de las colecciones, que almacenan elementos individuales, los mapas asocian cada clave con un valor correspondiente. Esto permite realizar búsquedas rápidas de valores basadas en sus claves.

### Características de los mapas

1. **Asociación Clave-Valor**: Cada elemento en un mapa se almacena como un par clave-valor. Las claves deben ser únicas, pero los valores pueden repetirse.
2. **Acceso Rápido**: Los mapas permiten acceder rápidamente a los valores mediante sus claves, lo que los hace eficientes para búsquedas.
3. **No Permiten Claves Duplicadas**: Un mapa no puede tener dos claves iguales. Si se intenta insertar una clave que ya existe, el valor asociado a esa clave se actualizará.

### Principales implementaciones de mapas

1. **HashMap**: Implementa la interfaz `Map` y utiliza una tabla hash para almacenar los pares clave-valor. No garantiza ningún orden específico de los elementos.
2. **TreeMap**: Implementa la interfaz `Map` y almacena los pares clave-valor en un árbol rojo-negro, lo que garantiza que los elementos estén ordenados según las claves.
3. **LinkedHashMap**: Implementa la interfaz `Map` y mantiene el orden de inserción de los elementos. Es útil cuando se necesita iterar sobre los elementos en el orden en que fueron insertados.
